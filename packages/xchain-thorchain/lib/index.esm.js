...
import { assetToString, THORChain as THORChain$1, assetFromString, baseAmount } from '@xchainjs/xchain-util';
...
var getDenomWithChain = function (asset) {
    return THORChain$1 + "." + asset.symbol.toUpperCase();
};
/**
...
var getAsset = function (denom) {
    if (denom === getDenom(AssetRune))
        return AssetRune;
    return assetFromString(THORChain$1 + "." + denom.toUpperCase());
};
/**
        ...
        msgs.map(function (msg) {
            if (isMsgSend(msg)) {
                ...
                var amount = msgSend_1.amount
                    .map(function (coin) { return baseAmount(coin.amount, DECIMAL); })
                    .reduce(function (acc, cur) { return baseAmount(acc.amount().plus(cur.amount()), DECIMAL); }, baseAmount(0, DECIMAL));
                var from_index_1 = -1;
                ...
                if (from_index_1 === -1) {
                    ...
                    from[from_index_1].amount = baseAmount(from[from_index_1].amount.amount().plus(amount.amount()), DECIMAL);
                }
                var to_index_1 = -1;
                ...
                if (to_index_1 === -1) {
                    ...
                    to[to_index_1].amount = baseAmount(to[to_index_1].amount.amount().plus(amount.amount()), DECIMAL);
                }
            }
                ...
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        result_1.observed_tx.tx.coins.forEach(function (coin) {
                            ...
                            asset_1 = assetFromString(coin.asset);
                        });
                        return [2 /*return*/, {
...
export { AssetRune, Client, DECIMAL, DEFAULT_GAS_VALUE, MAX_TX_COUNT, MSG_DEPOSIT, MSG_SEND, MsgNativeTx, THORChain, getAsset, getDefaultClientUrl, getDefaultExplorerUrls, getDefaultFees, getDenom, getDenomWithChain, getExplorerAddressUrl, getExplorerTxUrl, getExplorerUrl, getPrefix, getTxType, getTxsFromHistory, isBroadcastSuccess, isMsgMultiSend, isMsgSend, msgNativeTxFromJson, registerCodecs };
